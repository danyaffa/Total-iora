...
async function sttUpload(blob, mime, lang) {
  // send as base64 to /api/stt (server calls Whisper)
  const buf = await blob.arrayBuffer();
  let b64 = "";
  const bytes = new Uint8Array(buf);
  for (let i = 0; i < bytes.length; i++) b64 += String.fromCharCode(bytes[i]);
  const payload = { b64: btoa(b64), mime, lang: lang || "en" };
  const r = await fetch("/api/stt", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload) });
...
export default function OracleVoice({ path = "Universal" }) {
  ...
  function speak(text) {
    ...
    u.lang = chosenLang || "en-US"; u.volume = Math.max(0, Math.min(1, Number(volume) || 1));
  ...
  const audioRef = useRef({ ctx: null, analyser: null, src: null, animId: null, stream: null, rec: null, chunks: [] });
  useEffect(() => {
  ...
  function clearViz() {
    ...
    audioRef.current = { ctx: null, analyser: null, src: null, animId: null, stream: null, rec: null, chunks: [] };
  ...
  async function onStart() {
    ...
    try {
      ...
      // MediaRecorder for reliable STT on mobile
      ...
      rec.start(250); // small timeslice to keep buffer flowing
      ...
      audioRef.current = { ctx, analyser, src, animId: audioRef.current.animId, stream, rec, chunks, mime };
    } catch {
      setListening(false);
  ...
  async function onStop() {
    if (!listening) return;
    setListening(false);
    try { audioRef.current.rec?.stop?.(); } catch {}
    try { await new Promise(r => setTimeout(r, 120)); } catch {}
    const chunks = audioRef.current.chunks || [];
    const mime = audioRef.current.mime || "audio/webm";
    clearViz();
    if (!chunks.length) return;

    const blob = new Blob(chunks, { type: mime });
    const text = await sttUpload(blob, mime, chosenLang);
    if (text) setLiveText((prev) => (prev ? `${prev.trim()} ${text}` : text));
    // Do NOT auto-send — user presses "Get Answer"
  }

  async function sendForAnswer(text) {
    const clean = String(text || "").trim();
    if (!clean) return;
    // ignore 1–2 word accidents
    ...
    try {
      ...
      setReply(msg);
      if (msg) speak(msg);
      // show/hide sources remains as in your UI (data.sources may be empty)
      // If you render sources, keep it collapsed by default
    } finally {
      setReplying(false);
    }
  }
  ...
  return (
    <section className="oracle">
      <header className="head">
        ...
        <div style={{textAlign:"center", color:"#334155", fontWeight:700, marginTop:6}}>
          I only answer after you press <em>Get Answer</em>. Finish typing or dictating first.
        </div>
      </header>
      ...
      <div className="body">
        ...
        <div className="pane">
          ...
          <div className="log">
            ...
            {/* keep your sources UI; you can default it to hidden */}
          </div>
        </div>
      </div>

      <style jsx>{`
        .oracle{max-width:1100px;margin:16px auto;padding:18px 12px;border-radius:24px;background:
          radial-gradient(1200px 600px at 5% -10%, #eef2ff 0%, transparent 60%),
        ...
        .orb{width:140px;height:140px;min-width:140px;border-radius:999px;position:relative;background:
          radial-gradient(40% 40% at 50% 50%, rgba(124,58,237,.18), rgba(124,58,237,0));
