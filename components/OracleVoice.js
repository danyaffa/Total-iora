...
export default function OracleVoice({ path }) {
  ...
  function ensureRecognizer() {
    ...
    if (!SR) { alert("Voice recognition is not supported on this browser."); return null; }
    const rec = new SR();
    rec.lang = "en-US";
  ...
  async function onStart() {
    ...
    if (!rec) return;
    try { await startMicViz(); setListening(true); rec.start(); }
    catch { setListening(false); stopMicViz(); alert("Microphone permission denied or unavailable."); }
  }
