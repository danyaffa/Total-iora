...
// Sacred-text whitelist with strong filters (no CIA/.gov and no Gutenberg license boilerplate).
...
function allowSource(s) {
  const all = `${s.work||""} ${s.author||""} ${s.url||""} ${s.text||""}`.toLowerCase();
...
/* Christian & Eastern via Gutenberg (filtered) */
...
export default async function handler(req, res){
  ...
  try {
    ...
    if (path === "Christian") {
      tasks.push(fetchGutenbergTitle("King James Bible", Math.min(max,4)));
      tasks.push(fetchGutenbergTitle("The Imitation of Christ", 2));
      tasks.push(fetchGutenbergTitle("Confessions of Saint Augustine", 2));
    }
    if (path === "Eastern") {
      tasks.push(fetchGutenbergTitle("Tao Te Ching", 3));
      tasks.push(fetchGutenbergTitle("Bhagavad Gita", 3));
      tasks.push(fetchGutenbergTitle("Dhammapada", 3));
    }
    if (path === "Universal") {
      ...
      tasks.push(fetchSefaria(query, 2)); // include Hebrew wisdom occasionally
    }
