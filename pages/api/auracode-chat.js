...
| {};
  ...
  try {
    ...
    const sources = await retry(
      ...
      { retries: 2, minTimeout: 500, onRetry: (error) => { log.warn({ err: error }, 'Retrying source fetch...'); } }
    );
...
| response.choices.length === 0) {
      ...
      { retries: 2, minTimeout: 1000, onRetry: (error) => { log.warn({ err: error }, 'Retrying OpenAI chat completion call...'); } }
    );
