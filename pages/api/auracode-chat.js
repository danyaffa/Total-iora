...
// Sources allowed: Sefaria (Jewish), alquran.cloud (Qur'an), Gutenberg (KJV Bible, Tao Te Ching, Bhagavad Gita, Dhammapada).
...
function cleanPara(p){
  ...
  if (/[A-Z\s]{18,}/.test(s) && !/[a-z]/.test(s)) return ""; // all-caps headings
  return s;
}
...
async function fetchSefariaSnippets(query, topK=6) {
  try {
    ...
    for (const h of hits) {
      ...
      const quote = cleanPara(en) || String(en||"").slice(0,800);
      if (!quote) continue;
...
async function fetchQuranSnippets(query, topK=6){
  try {
    ...
    return matches.slice(0, topK).map(m => ({
      ...
      quote: String(m.text || "").slice(0,900),
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // gather approved sources only (based on room/path)
    const tasks = [];
