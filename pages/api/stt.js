...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Build multipart form for Whisper/gpt-4o-mini-transcribe
    const form = new FormData();
    form.append("file", new File([buf], `speech.${ext}`, { type }));
    ...
    if (!r.ok) return res.status(502).json({ error: "Transcription failed", detail: js });
