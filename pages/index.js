...
// --- VIEW 1: The Active/Unlocked App for Registered Users ---
const ActiveApp = () => {
  const [path, setPath] = useState("Universal");

  const handleLogout = () => {
    deleteCookie("ac_session");
    window.location.reload(); // Reload the page to show the static view
  };
...
// --- VIEW 2: The Static Landing Page for New Visitors ---
const StaticPage = () => {
  return (
    <>
      <nav className="topnav">
...
export default function Home() {
  ...
  useEffect(() => {
    // This logic runs in the browser to check if the user is registered.
    ...
    // The ONLY check that matters for a real user: the login session cookie.
  ...
  // Show a blank or loading state while the cookie is being checked
  if (loading) {
  ...
  return (
    <div className="page">
      {/* This is the core logic: show the correct view based on the 'unlocked' state */}
      {unlocked ? <ActiveApp /> : <StaticPage />}
      
      <Footer />

      {/* The exact styles from your file are preserved below */}
      <style jsx>{`
        .page { min-height:100vh; background:linear-gradient(#ffffff,#f8fafc); }

        .topnav { display:flex; justify-content:center; padding:14px; }
        .btn { display:inline-block; padding:10px 16px; border-radius:14px; font-weight:800; border:1px solid rgba(15,23,42,.12); background:#fff; text-decoration: none; cursor: pointer; text-align: center;}
        .btn.cta { color:#fff; border:none; background:linear-gradient(135deg,#7c3aed,#14b8a6); }
        .btn.ghost { background: transparent; }

        /* The nav for the logged-in view is right-aligned */
        .page > :global(.topnav) { justify-content: flex-end; }
        ...
        .card { background:#fff; border:1px solid rgba(15,23,42,.08); border-radius:20px; box-shadow:0 10px 30px rgba(2,6,23,.08); padding:18px; display: flex; flex-direction: column; justify-content: space-between; }
        .h { flex-grow: 1; }
        ...
        p { color:#475569; }
        .f { display:flex; flex-direction:column; gap:8px; margin-top:1.5rem; }
